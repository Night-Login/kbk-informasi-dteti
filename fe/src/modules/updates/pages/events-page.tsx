...
function EventCalendar({ events }: { events: WebsiteEvent[] }) {
  ...
  useEffect(() => {
    ...
    const now = Date.now();
    const target = events.find((event) => new Date(event.starts_at).getTime() >= now) || events[0];
  ...
  return (
    <aside className="rounded-xl border border-line bg-surface p-5 sm:p-6 xl:sticky xl:top-28">
      ...
      <p className="mt-4 text-sm font-bold text-dteti-blue">{monthFormatter.format(month)}</p>
      <div className="mt-4 grid grid-cols-7 gap-1 text-center text-xs">
        {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => (
          <div key={day} className="py-2 font-bold text-muted">{day}</div>
        ))}
        {cells.map((day, index) => {
    ...
    <main id="main-content" className="min-h-screen bg-white pb-20 pt-24 text-ink sm:pt-28">
      <div className="page-container">
        ...
        <header className="mx-auto max-w-3xl pb-8 pt-7 text-center">
          <h1 className="text-4xl font-bold tracking-[-0.025em] text-dteti-ink sm:text-5xl">Events</h1>
          <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-muted">
            Seminars, discussions, workshops, and academic activities from the Information Engineering Research Group.
          </p>
        </header>
        ...
        {loading ? (
          ...
          <div className="mt-10 grid gap-10 xl:grid-cols-[minmax(0,1.3fr)_minmax(22rem,0.7fr)] xl:items-start">
            {visibleItems.length > 0 ? (
              <section aria-label="Events" className="grid gap-6 sm:grid-cols-2">
                {visibleItems.map((event, index) => {
                  const start = new Date(event.starts_at);
                  const end = event.ends_at ? new Date(event.ends_at) : null;
                  const image = getApiAssetUrl(event.media?.file_url || event.image_url);
                  return (
                    <article id={event.slug} key={event.id} className="flex scroll-mt-28 flex-col overflow-hidden rounded-xl border border-line bg-white">
                      {image ? (
                      ...
                      <div className="flex flex-1 flex-col p-6">
                        <time dateTime={event.starts_at} className="text-xs font-bold uppercase tracking-wider text-dteti-blue">
                          {dateFormatter.format(start)}
                        </time>
                        <h2 className="mt-3 text-xl font-bold leading-7 text-dteti-ink">{event.title}</h2>
                        {event.description ? <p className="mt-3 line-clamp-4 text-sm leading-6 text-muted">{event.description}</p> : null}
                        <dl className="mt-5 grid gap-3 border-t border-line pt-4 text-xs">
                          <div className="flex gap-2.5">
                            <Clock3 size={16} className="mt-0.5 flex-none text-dteti-blue" aria-hidden="true" />
                            <div>
                              <dt className="sr-only">Time</dt>
                              <dd className="font-semibold text-ink">
                                {timeFormatter.format(start)} WIB{end ? ` – ${timeFormatter.format(end)} WIB` : ""}
                              </dd>
                            </div>
                          </div>
                          <div className="flex gap-2.5">
                            <MapPin size={16} className="mt-0.5 flex-none text-dteti-blue" aria-hidden="true" />
                            <div>
                        ...
                        {event.link_url ? (
                          <Link href={event.link_url} className="mt-5 inline-flex items-center gap-1.5 self-start text-sm font-bold text-dteti-blue hover:underline">
              ...
              <div className="border border-line bg-surface px-6 py-14 text-center">
                ...
                <p className="mt-2 text-sm text-muted">
                  {items.length > 0 ? "Try another keyword or event status." : "Published events will appear here."}
                </p>
              </div>
            )}
