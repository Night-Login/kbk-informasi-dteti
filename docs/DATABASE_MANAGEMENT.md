...
Publications use an internal UUID `id` as their primary key. DOI is optional because crawler output does not always contain one; a DOI must still be unique when it is present. Lecturer-publication relations store `publication_id`, so adding or correcting a DOI does not change relation keys.

CSV/JSON imports match an existing publication by DOI first and then by a valid source UUID. Keep crawler UUIDs stable between data drops. A row without either DOI or UUID falls back to its slug, which is intended only for simple manual imports.
...
- **Upsert** (Update or Insert) Lecturers by their lecturer identifiers and Publications by DOI or stable UUID without rejecting publications that have no DOI.
- Resolve lecturer-publication links to UUID `publication_id`, with compatibility for `publication_doi`/`doi` references when present.
